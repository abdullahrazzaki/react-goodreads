  ...
  if (keyword) {
    axios
      ...
      .then(response => {
        ...
        let count = parseInt(
          obj.children[1].getElementsByTagName("total-results")[0].value
        );
        let result = {
          suggestions: suggestions,
          remainingCount: count - suggestions.length
        };
    ...
    axios
      ...
      .then(response => {
        ...
        const currentOffset = parseInt(
          obj.children[1].getElementsByTagName("results-end")[0].value
        );
        const nextPage = currentOffset < count ? page + 1 : -1;
        const result = { results: suggestions, nextPage: nextPage };
        console.log(suggestions);
        res.json(result);
      })
      .catch(res => console.log(res));
...
app.get("/book", function(req, res) {
  ...
  if (id) {
    axios
      .get(`https://www.goodreads.com/book/show`, {
        params: {
          key: "EcXsQYe942QoFS6FFq5Mw",
          search: "title",
          id: id,
